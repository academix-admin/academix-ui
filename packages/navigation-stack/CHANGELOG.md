...
- **1.0, and what that promises.**

  Nothing in this release breaks. The version changes because the number was saying something untrue:
  `0.x` tells a reader "this will change under you", and the public surface has been stable for
  months while the changes underneath were fixes to behaviour the surface already described. A
  version that undersells stability costs exactly as much as one that oversells it — it is just
  cheaper to be wrong in this direction, which is not a reason.

  **What is covered.** Everything exported from `@academix-admin/navigation-stack` and its entry
  points `/devtools`, `/testing` and `/playwright`. That surface is not a matter of opinion: it is
  written down in `test/public-api.test.ts`, which fails if a name appears or disappears without
  somebody meaning it.

  - **Patch** — a fix, with the surface unchanged.
  - **Minor** — something added. Existing code keeps working.
  - **Major** — something removed, renamed, or made to behave differently on purpose.

  **What is not covered**, and should not be relied on:

  - Anything under `src/core/*`, `src/scroll/*` and the rest. Tests in this repo import them; your
    app should not.
  - The exact text of an entry's uid. It names a place — group, stack, page, position — and the
    format may change in a major. Stacks persisted by an older version are regenerated by position
    on the way in, so an upgrade does not lose anybody's place.
  - Timing. When an effect runs relative to a transition is not a contract.

  **Deferred rather than done:** `NavigationDevtools` is still exported from the main barrel as well
  as from `/devtools`, which is why the main bundle is 42.1 KB gzipped rather than 38.9 KB. Removing
  it is a breaking change, and the app that imports it that way is eight minor versions behind and
